...
struct Node * search_node(struct Node *tree, int valor, struct Node *result);
...
int main(void)
{
	...
	if (search_node(root, num_search, result) != NULL) {
...
struct Node * search_node(struct Node *tree, int value, struct Node *result)
{ 
	...
	if (tree != NULL) {
		if (value == tree->num) {
			...
			result = search_node(tree->node_left, value, result);

			if (result == NULL) {
				result = search_node(tree->node_right, value, result);
